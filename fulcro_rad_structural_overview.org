...
- High-level discussion of transit middlware
- High-level discussion of file upload middlware
...
- Attributes also include things like style, which is a hint to the rendering
  ...
  can set as ao/style.
- {ao/required?} Hint to the plugin laywers if this attribute is required.
